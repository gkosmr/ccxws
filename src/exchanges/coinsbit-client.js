...
class CoinsbitClient extends BasicClient {
  /**
    Documentation:
    https://www.notion.so/coinsbitwsapi/API-COINSBIT-WS-API-COINSBIT-cf1044cff30646d49a0bab0e28f27a87
   */
  constructor({ wssPath = "wss://ws.coinsbit.io/", watcherMs } = {}) {
    super(wssPath, "Coinsbit", undefined, watcherMs);
  ...
  _onMessage(msg) {
    ...
    if(message.result == 'pong') {
      ...
      let market = this._tradeSubs.get(message.params[0]);
      if(market) {
